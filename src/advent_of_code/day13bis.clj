...
(defn- find-safe-entry
  ...
  [schedule]
  (let [;; Start by stripping `schedule` down to just level and cycle
        sched (reduce (fn [sch [depth [_ cycle]]]
                        (assoc sch depth cycle)) {} schedule)
        test  (fn [wait]
                (not-any? #(zero? (mod (+ % wait) (sched %))) (keys sched)))]
    (first (filter test (range)))))
